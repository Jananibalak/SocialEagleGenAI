...
const { user, tokenBalance, freeMessagesLeft, logout, loadTokenBalance } = useAuth();
...
const [statsLoading, setStatsLoading] = useState(true);
 ...
 // ✅ REPLACE: Load data on screen focus
useFocusEffect(
  React.useCallback(() => {
    loadPreferences();
    loadStats();
...
const loadPreferences = async () => {
  try {
    const prefs = await AsyncStorage.getItem('@edupath_user_preferences');
    if (prefs) {
...
const loadStats = async () => {
  try {
    ...
    // Fetch mentors
    const mentorsResponse = await apiService.listMentors();
    const mentors = mentorsResponse.mentors || [];
    
    // Count messages
    let totalMessages = 0;
    const allKeys = await AsyncStorage.getAllKeys();
    const chatKeys = allKeys.filter(key => key.startsWith('@edupath_chat_'));
    
    for (const key of chatKeys) {
      try {
        const messages = await AsyncStorage.getItem(key);
        if (messages) {
          totalMessages += JSON.parse(messages).length;
        }
      } catch (err) {
    ...
    mentors.forEach(mentor => {
      if (mentor.streak && mentor.streak > maxStreak) {
        maxStreak = mentor.streak;
      }
    });
...
const handleLogout = () => {
  ...
  showAlert(
    'Sign Out',  // ← Keep your original text
    'Are you sure you want to sign out?',  // ← Keep your original text
    [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Sign Out',  // ← Keep your original button text
        style: 'destructive',
        onPress: async () => {
          ...
          if (result.success) {
            ...
            // Still show login even on error since user state is cleared
            console.log('Navigation should still happen');
          }
        },
      },
    ]
        ...
        <Animated.View entering={FadeInDown.duration(500)} style={styles.profileSection}>
          ...
          <View style={styles.tokenContainer}>
            ...
            >
              ...
              <Text style={styles.tokenText}>{statsLoading ? '...' : (tokenBalance?.toFixed(0) || '0')} Tokens</Text>
            </LinearGradient>
          </View>
        </Animated.View>

        <View style={styles.statsContainer}>
          <StatCard icon="users" label="Mentors" value={statsLoading ? '...' : stats.mentorCount} index={0} />
          <StatCard icon="message-circle" label="Scrolls" value={stats.scrollsCount} index={1} />
          <StatCard icon="zap" label="Streak" value={statsLoading ? '...' : stats.dayStreak} index={2} />
        </View>
